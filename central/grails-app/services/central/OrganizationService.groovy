...
class OrganizationService {
    ...
    Organization create(Map params) {
        ...
        if (!organization.save(flush:true)) {
            organization.errors.allErrors.each {
                log.warning ("create: error while saving organization ${organization}: ${error}")
            }
        }
    ...
    @PreAuthorize("hasPermission(#id, 'central.Organization', read) or hasPermission(#id, 'central.Organization', admin)")
