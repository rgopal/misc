...
class PersonRoleInstanceService {
    ...
    PersonRole getNew(Map params) {
        ...
        if (!personRole.person) {
            personRole.errors.allErrors.each {
                log.warning ("create: error while getting new personRole ${personRole}: ${it}")
            }
        }
        personRole
    }
    ...
    PersonRole create(Map params) {
        ...
        if (!personRole.save(flush:true)) {
            personRole.errors.allErrors.each {
                log.warning ("create: error while saving personRole ${personRole}: ${it}")
            }
        }
