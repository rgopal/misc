 ...
 */
@Log4j
class InitPersonContentAssessment {
    ...
    void load () {
        ...
        def personAssessment = personInstruction.personAssessments[0]
          ...
          if (!personAssessment)
            log.warn "load: personAssessment not found for classSession Computer Science I - Fall 2015 Class Session"
        ...
        for (personContent in personContents) {     
           log.trace "processing  personContent ${personContent} for personAssessment"
            
            personAssessment.addToPersonContents(personContent)
        ...
        log.info ("load: loaded ${PersonContent.countByPersonAssessment(personAssessment)} " +
                " out of ${personContents.size()} personContents for personAssessment")
