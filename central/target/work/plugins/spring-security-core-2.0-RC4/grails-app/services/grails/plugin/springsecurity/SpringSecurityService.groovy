...
class SpringSecurityService {
    ...
    Object getCurrentUser() {
        ...
        // println "SSS.g: getCurrentUser User $User"
        if (principal instanceof GrailsUser) {
            ...
            // println "SSS.g: getCurrentUser uPN $usernamePropName principal $principal"
            // with admin logged on through software there is no username and no id
            // so changed the following to just principal
            // User.findWhere((usernamePropName): principal.username) 2015/04/28
            
            User.findWhere((usernamePropName): principal)
        }
    }
